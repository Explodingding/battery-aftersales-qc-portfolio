...
async function renderGateCards() {
  const el = document.getElementById("gate-cards");
  if (!el) return;
  try {
    const res = await fetch("data/gate_methods.json");
    const data = await res.json();
    el.innerHTML = data.gates.map((g) =>
      `<article class="gate-card"><h4><span class="gate-num">${g.step}</span>${g.name}</h4>`
      + `<dl><dt>Time box</dt><dd>${g.timebox_min ? g.timebox_min + " min" : "Per WI"}</dd>`
      + `<dt>Pass</dt><dd>${g.pass}</dd><dt>If fail</dt><dd>${g.fail_action}</dd>`
      + `<dt>Why</dt><dd>${g.plain_why}</dd></dl></article>`
    ).join("");
  } catch (_) {
    el.innerHTML = "";
  }
